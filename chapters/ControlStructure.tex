...
where $\mathcal{S}$ denotes the spectral radius - the absolute value of the largest eigenvalue - and $\otimes$ is the Kronecker product. Applying this theorem, we can show that the VFLQR system is stable for the given values of packet loss, with spectral radii of $\{0.9238,0.9271,0.9453\}$. In fact, applying Theorem 6 in \cite{Hu2007}, we can show that the VFLQR system should be stable for \textit{any} value of packet loss, as the largest positive eigenvalue of the matrix:
...
is $\text{max}(\sigma(V)) = 0.81$ and thus $PDM = \frac{1}{\text{max}(\sigma(V))} = 1.24$.
